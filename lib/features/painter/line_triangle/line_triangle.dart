...
class LineWithTrianglePainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    ...
    Offset startPoint = const Offset(50, 150);
    Offset endPoint = const Offset(250, 150);

    // Draw the line
    canvas.drawLine(startPoint, endPoint, paint);

    // Draw the triangle (arrowhead) at the end of the line
    drawTriangle(canvas, paint, endPoint, startPoint);
  }

  void drawTriangle(Canvas canvas, Paint paint, Offset tip, Offset tail) {
    const double triangleSize = 15.0;

    // Calculate the angle of the line (direction)
    double angle = atan2(tip.dy - tail.dy, tip.dx - tail.dx);
